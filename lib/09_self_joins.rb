...
def connecting_routes
  ...
  # that link these stops that have a count of 2. Add a HAVING clause to restrict
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    WHERE
      a.stop_id = 230 AND b.stop_id = 53;
  SQL
end
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT end_route_stops.name, end_routes.company, end_routes.num
    FROM
